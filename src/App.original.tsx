...
import { isNavTrainingComplete } from './components/NavigationTraining'

// ─── App Phase: welcome → calibration → course-preview → layout-challenge → henderson-challenge → course-selection → training ───
type AppPhase = 'welcome' | 'calibration' | 'challenge-notice' | 'course-preview' | 'layout-challenge' | 'henderson-challenge' | 'course-selection' | 'nav-training' | 'training'
...
export default function App() {
  ...
  const navigateFromDock = (target: DockNavigationTarget) => {
    switch (target) {
      ...
      case 'night-card':
        ...
        return
      case 'light-card':
        // If nav-training not yet completed, intercept and teach first
        if (!isNavTrainingComplete()) {
          setAppPhase('nav-training')
          return
        }
        setAppPhase('training')
        // lightCardInitialIndex is set before this call if provided
        setViewMode('LC')
        return
      case 'night-web':
        setAppPhase('training')
  ...
  const parseDockTarget = (): { target: DockNavigationTarget; cardIndex?: number } | null => {
    const hash = window.location.hash ?? ''
    const queryStart = hash.indexOf('?')
    if (queryStart === -1) return null
    const params = new URLSearchParams(hash.slice(queryStart + 1))
    const rawTarget = params.get('dock')
    const rawCardIndex = params.get('cardIndex')
    ...
    const target = allowedTargets.includes(rawTarget as DockNavigationTarget)
      ...
      : 'light-card'
    const cardIndex = rawCardIndex ? parseInt(rawCardIndex, 10) : undefined
    return { target, cardIndex: Number.isFinite(cardIndex) ? cardIndex : undefined }
  }

  useEffect(() => {
    const applyDockFromHash = () => {
      const result = parseDockTarget()
      if (!result) {
      ...
      // Set cardIndex if provided
      if (result.target === 'light-card' && result.cardIndex !== undefined) {
        setLightCardInitialIndex(result.cardIndex)
      }
      navigateFromDock(result.target)
    }

    const handleDockNav = (event: Event) => {
      const detail = (event as CustomEvent<DockNavigationTarget | { target: DockNavigationTarget; cardIndex?: number }>).detail
      if (!detail) return
      // Handle both simple string and object with cardIndex
      if (typeof detail === 'string') {
        navigateFromDock(detail)
      } else if (typeof detail === 'object' && detail.target) {
        if (detail.target === 'light-card' && detail.cardIndex !== undefined) {
          setLightCardInitialIndex(detail.cardIndex)
        }
        navigateFromDock(detail.target)
      }
    }
    ...
    return (
      <Suspense fallback={loadingFallback}>
        ...
        <PreChallengeCoursePage
          ...
          onBack={() => setAppPhase('challenge-notice')}
    ...
    return (
      <Suspense fallback={loadingFallback}>
        <NavigationTraining
          theme={theme === 'night' ? 'night' : 'day'}
          onComplete={() => {
            // Nav training done — proceed to light-card training
            setViewMode('LC')
            setAppPhase('training')
          }}
        />
